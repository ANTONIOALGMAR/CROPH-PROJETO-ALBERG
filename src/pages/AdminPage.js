...
const AdminPage = () => {
  ...
  const [mainTab, setMainTab] = useState('presenca');
  ...
  const handleRegisterParticipation = useCallback(
    async (leito, tipoEvento, participou, dataEvento) => {
      if (!jwtToken) return;

      const conviv = conviventes.find((c) => c.leito === leito);
      const conviventeId = conviv ? conviv.id : null;
      let url;
      let body = { leito, data: dataEvento, conviventeId, participou };
    ...
    <div className="bg-blue-100 min-h-screen p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-4xl font-bold text-gray-800">Página do Administrador</h1>
          <button
            onClick={() => {
              localStorage.clear();
              navigate('/login');
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          {mainTab === 'refeicoes' && (
            <div>
              ...
              <RefeicaoGrid
                tipoRefeicao={refeicaoTab.toUpperCase()}
                conviventes={conviventes}
                dataSelecionada={selectedDate}
                onRegisterParticipation={handleRegisterParticipation}
                token={jwtToken}
          ...
          {mainTab === 'cadastroUsuarios' && <UserRegistrationForm />}
        </div>
      </div>
    </div>
  );
